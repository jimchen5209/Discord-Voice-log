...
## 2. Technical Changes
- **Database**: MongoDB -> SQLite (`dev.db`)
...
2. **Prepare MongoDB Dump**:
   ...
   *Note: If your dump file has a different name or location, you can pass the path via an environment variable when starting the bot:*
   ```bash
...
4. **Verification**:
   After the bot starts, check if the `dev.db` file contains the migrated data using Prisma Studio:
   ```bash
   npx prisma studio
   ```
